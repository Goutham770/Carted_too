...
const Cart = () => {
  ...
  return (
    <div className="px-8 lg:px-16 pt-10 font-poppins pb-10">
      <h1 className="text-5xl mb-4  text-green-400 font-poppins font-bold">Your Cart</h1>
        ...
        <div>
            ...
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between lg:justify-start lg:space-x-10 space-x-2">
                  ...
                  >
                    <AiOutlineDelete className="inline-block mr-1" /> {/* Delete icon */}
                    Remove from Cart
                  </button>
                  <div className="flex">
                    <button
                      className="bg-white hover:scale-90 font-bold px-2 text-red-600 border-red-500 lg:px-2 rounded-lg lg:text-2xl"
                      onClick={() =>
                        dispatch({
                    ...
                    <button
                      className="bg-white hover:scale-90 font-bold px-2 text-green-600 lg:px-2 rounded-lg lg:text-2xl"
                      onClick={() =>
                        dispatch({
                          type: "UPDATE_QUANTITY",
                          payload: { id: item.id, quantity: item.quantity + 1 },
          ...
          <div className="mt-6 space-y-4">
            <h1 className="lg:text-xl">
              Subtotal:{" "}
              <span className="font-bold text-orange-400">${totalPrice.toFixed(2)}</span>{" "}
            </h1>
            <h1 className="lg:text-xl">
              Final Price (10% discount):{" "}
              <span className="font-bold text-orange-500">${finalPrice.toFixed(2)}</span>
            </h1>
          </div>
        </div>
      )}
